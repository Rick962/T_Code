...
def analys_pub(item):
    try:
        ...
        if 'tick' in item.keys():
            ...
            if symbol[-3:] == 'btc':
                try:
                    exrate = float(redisConnect.get('vb:ticker:rmbnewprice:btc_usdt'))
                except:
                    exrate = float(70000)
                cnyP = close * exrate
                ...
                try:
                    exrate = float(redisConnect.get('vb:ticker:rmbnewprice:eth_usdt'))
                except:
                    exrate = float(1400)
                cnyP = close * exrate
            else:
            ...
            # print(dataArr)
            redisConnect.set('vb:ticker:rmbnewprice:' + str(code), cnyP)
