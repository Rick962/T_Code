...
def write_db(data):
    try:
        insert_sql = "REPLACE INTO xy_15min_info(pid, code, name, openingPrice, highestPrice, " \
            ...
            if coinMap.get(code)['old_time_index'] is None:
                ...
                if coinMap.get(code)['old_time_index'] != new_time_index:
                    ...
                    if volumes < 0:
                        ...
                        volumes = abs(int(coinMap.get(code)['volume'] / 96))
                    ...
                    redisConnect.publish('vb:channel:newkline:minute15', json.dumps(dataArr))
