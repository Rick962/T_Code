...
def write_db(data):
    try:
        insert_sql = "REPLACE INTO xy_60min_info(pid, code, name, openingPrice, highestPrice, " \
...
while True:
    ...
    for i in message:
        if i["type"] == 'message':
            ...
            new_time_index = int(time.strftime("%H", time.localtime(int(timestamp))))
            # new_time_index = index - index % 30
            datetimes = time.strftime("%Y-%m-%d %H:00:00", time.localtime(timestamp))
            ...
            if coinMap.get(code)['old_time_index'] is None:
                ...
                if coinMap.get(code)['old_time_index'] != new_time_index:
                    ...
                    if volumes < 0:
                        volumes = abs(int(coinMap.get(code)['volume']/24))
                    ...
                    redisConnect.publish('vb:channel:newkline:minute60', json.dumps(dataArr))
